...
public class TransformerMacros {
    public static void LogMessage(InsnList list, String level, String message) {
        ...
        instructions.add(new FieldInsnNode(Opcodes.GETSTATIC, "xyz/lilyflower/lilytweaks/core/LilyflowerTweaksBootstrapSystem", "LOGGER", "Lorg/apache/logging/log4j/Logger;"));
    ...
    public static void GetStaticField(Class<?> clazz, String name, InsnList list) {
        try {
            ...
            LoggingHelper.oopsie(LilyflowerTweaksBootstrapSystem.LOGGER, "FAILED GETTING FIELD: " + name, exception);
        }
    }

    public static boolean CheckStaticCall(Class<?> clazz, String name, Class<?>[] arguments, MethodInsnNode node) {
        try {
            String owner = Type.getInternalName(clazz);
            Method method = clazz.getDeclaredMethod(name, arguments);
            String descriptor = Type.getMethodDescriptor(method);

            if (LilyflowerTweaksTransformerLoadingSystem.DEBUG_ENABLED) LilyflowerTweaksBootstrapSystem.LOGGER.debug("Validating {}#{}{} against {}#{}{}", owner, name, descriptor, node.owner, node.name, node.desc);
            return node.owner.equals(owner) && node.desc.equals(descriptor);
        } catch (Throwable exception) {
            LoggingHelper.oopsie(LilyflowerTweaksBootstrapSystem.LOGGER, "FAILED GETTING CALL: " + name, exception);
        }
