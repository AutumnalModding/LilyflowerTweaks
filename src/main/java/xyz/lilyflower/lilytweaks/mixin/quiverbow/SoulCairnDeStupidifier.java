package xyz.lilyflower.lilytweaks.mixin.quiverbow;
...
public class SoulCairnDeStupidifier {
    @Unique
    private static final HashMap<String, EggLogic> lilyflower$MOD_SPAWN_EGGS = new HashMap<>();
    ...
    public void onImpact(MovingObjectPosition target) {
        ...
        for (String blacklisted : QuiverbowIntegrationConfiguration.ENTITY_BLACKLIST) {
            if (!allowed) break;
            try {
        ...
        if (allowed) {
            ...
            Optional<EggLogic> logic = lilyflower$MOD_SPAWN_EGGS.entrySet()
                    .stream()
            ...
            if (stack.getDisplayName().equals("Spawn")) {
                ...
                if (shot.shootingEntity instanceof EntityPlayerMP player) {
                    player.addChatMessage(new ChatComponentTranslation("lilytweaks.invalid_spawn_egg.1").setChatStyle(new ChatStyle().setColor(EnumChatFormatting.RED)));
                    player.addChatMessage(new ChatComponentTranslation("lilytweaks.invalid_spawn_egg.2").setChatStyle(new ChatStyle().setColor(EnumChatFormatting.RED)));
                    player.addChatMessage(new ChatComponentTranslation("lilytweaks.invalid_spawn_egg.3").setChatStyle(new ChatStyle().setColor(EnumChatFormatting.RED)));
                    player.addChatMessage(new ChatComponentTranslation("lilytweaks.invalid_spawn_egg.4").setChatStyle(new ChatStyle().setColor(EnumChatFormatting.RED)));
                }
            } else {
                entity.setDead();
            }
    ...
    static {
        lilyflower$MOD_SPAWN_EGGS.put("alfheim.common.entity", new EggLogic.AlfheimEggLogic());
        lilyflower$MOD_SPAWN_EGGS.put("lotr.common.entity", new EggLogic.GenericEggLogic("lotr", "item.spawnEgg"));
        lilyflower$MOD_SPAWN_EGGS.put("twilightforest.entity", new EggLogic.GenericEggLogic("TwilightForest", "item.tfspawnegg"));
        lilyflower$MOD_SPAWN_EGGS.put("com.gildedgames.the_aether.entities", new EggLogic.GenericEggLogic("aether_legacy", "aether_spawn_egg"));
    }
}
